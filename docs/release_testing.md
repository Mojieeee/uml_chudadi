...
| 客户端断线 | 房主提示断线，座位显示托管，房主 AI 接管该座位 | 待复测 |
| 客户端重连 | 当前版本按重新加入处理，不承诺自动恢复原座位 | 待复测 |
| 房主断线 | 客户端弹窗提示房主连接断开，退出当前房间并提示重新创建或重新加入 | 待复测 |
...
7. 多机环境可用时，补做创建房间、加入房间、添加人机、满座准备、开局、出牌同步、客户端断线托管、房主断线退出提示验收。
...
- 蓝牙断线恢复采用保守方案：客户端断线由房主 AI 托管；房主断线后客户端退出当前对局并提示重新创建或重新加入。
- 结束后战绩、金币和胜率会保存到本机。
- 玩家界面保持发布版，不展示课程交付文字。
