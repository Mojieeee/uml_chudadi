...
| `GameMessageCodecTest` | ROOM、START、MOVE、SNAPSHOT、DISCONNECT_NOTICE 等消息往返 |
| `RoomSeatTest` | 四座位、添加人机、准备、客户端断线托管、再开局重置 |
...
5. 游戏中断开一个客户端，确认房主显示断线提示、座位进入“托管”、AI 继续代打。
6. 使用同一设备重新加入时，按普通加入流程处理；当前版本不承诺自动恢复原座位。
7. 游戏中断开房主，确认客户端弹窗提示房主连接已断开，并退出当前房间，提示重新创建或重新加入。
8. 对局结束后点击再来一局，好友局回原房间，人机局回到预备牌桌，点击开始后重新发牌。
9. 拒绝蓝牙权限、关闭蓝牙或连接失败时，确认页面给出恢复提示。
10. 进入玩家中心，验证改名、头像解锁、自定义头像、每日奖励和重置战绩均先弹出确认或结果弹窗。
11. 从开屏页进入大厅，检查开屏牌面、按钮和主视觉动画无明显错位。

## 遗留风险

- 蓝牙真实多机环境受手机品牌、系统权限、配对状态和距离影响，仍需最终真机复测；当前版本不实现自动断线重连和房主迁移，房主断线后以退出并重新建房作为恢复路径。
